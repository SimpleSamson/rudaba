//this file desribes the motion of robot parts/points in 3d space using 3D points and speed(the speed will be translated by a motor controller)
...
struct Motion{
//    start and end points should be here
    start: ThreedPoint,
    end: ThreedPoint,
    speed: f64,
    direction: f64, //this can be changed depending on the joint and their combination
    time: f64   //this is the time duration that a motion should occur
}
...
fn main() {
    ...
    let boxed_cube: Box<Cube> = Box::new(
        Cube{
            top_left_front: three_d_origin(),
            bottom_right_front: ThreedPoint{x:3.0, y: -4.0, z: 0.0},
            top_left_back: ThreedPoint{x:0.0, y: 0.0, z: -3.0},
